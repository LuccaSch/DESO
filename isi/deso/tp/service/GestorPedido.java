...
public class GestorPedido {
    ...
    public Pedido crearPedido(int id, Cliente cliente, List<ItemPedido> pedidoDetalle) {
        ...
        try {
            ...
            gestorCliente.agregarPedido(cliente, pedido);
        } catch (VendedorNoUnicoException excep) {
            System.err.println("Desde GestorPedido: " + excep.getMessage());
        }
        return pedido;
    }
    ...
    public Pedido convertirPedidoDesdeDTO(PedidoDTO pedidoDTO) {
        ...
        try {
            vendedorEsUnico(pedidoDetalle);
            pedidoNuevo = crearPedido(pedidoDTO.getId(), gestorCliente.getClienteDAO().buscarPorIdCliente(pedidoDTO.getIdCliente()), pedidoDetalle);
            gestorCliente.agregarPedido(pedidoNuevo.getCliente(), pedidoNuevo);
    ...
    public double aplicarRecargo(Pedido pedido) {
        ...
        System.out.println("Desde GestorPedido: El cliente realiza el pago de $" + precioTotal + " con " + pedido.getContextoPago().getPagoStrategy().toString());
    ...
    public double aplicarRecargoPorDTO(PedidoDTO pedidoDTO) {
        ...
        try {
            ...
            System.out.println("Desde GestorPedido: " + exception.getMessage());
        }

        if (contextoPago != null) {
            double precioTotal = contextoPago.agregarRecargo(pedidoDTO.getPrecioTotal());
            System.out.println("Desde GestorPedido: El cliente realiza el pago de $" + precioTotal + " por " + contextoPago.getPagoStrategy().nombreEstrategia());
            return precioTotal;
        } else {
            return -1;
        }
